...
class Input(Form):
    ...
    opponent = SelectField(
        ...
        validators=[
            validators.InputRequired(),
            validators.AnyOf(
                ["Computer (easy)", "Computer (hard)", "Human"],
...
class ConnectK(object):
    ...
    def ResetBoard(self):
        ...
        self.board_display_ = [
            ["" for j in range(self.n_)] for i in range(self.m_)
        ]
    ...
    def UpdateDisplay(self):
        ...
        # update the display
        self.m_ = max(
            max([len(col) + 1 for col in self.board_.values()]), self.m_
        )
        ...
        if mode == "hard":
            ...
            for j in range(l, r + 1):
                ...
                computer_winner, _ = self.CheckForGameOver(
                    self.opponent_color_, forecast=True
                )
                self.UnplayMove()
                if computer_winner:
                    return j
            ...
            for j in range(l, r + 1):
                ...
                player_winner, player_winning_move_i = self.CheckForGameOver(
                    self.player_color_, forecast=True
                )
                self.UnplayMove()
                self.ToggleCurrentPlayer()
                if player_winner:
            ...
            for j in range(l, r + 1):
                ...
                score = max(
                    0.0 * my_contiguous_blocks
                    - 1.0 * opponent_contiguous_blocks
                    + 1e-5
                    * (
                        len(self.board_[j]) >= 2
                        and self.board_[j][1] == opponent
                    ),
                    best_score_so_far,
                )
                ...
                best_move = (
                    j
                    if score > best_score_so_far and not player_winner
                    else best_move
                )
                self.UnplayMove()
    ...
    def CountAdjacentBlocks(self, player):
        ...
        # horizontal check
        i = 0
        while self.board_[last_move] and i < len(self.board_[last_move]):
